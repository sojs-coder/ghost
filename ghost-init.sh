#!/usr/bin/env bash
# ghost-init.sh — Ghost Device environment initializer
# Installs dependencies, starts usbmuxd, establishes DVT tunnel,
# then hands off to ghost-run.py to play a GPX route file.
#
# Usage:
#   ./ghost-init.sh <route.gpx>
#   ./ghost-init.sh <route.gpx> --speed 2        # 2x speed
#   ./ghost-init.sh <route.gpx> --no-loop        # play once then hold last position
#   ./ghost-init.sh <route.gpx> --skip-deps      # skip apt/pip checks (faster re-runs)
...
RUNNER="$(dirname "$(realpath "$0")")/ghost-run.py"
...
Options:
  ...
  --skip-deps     Skip apt/pip dependency checks on subsequent runs
  --help
...
install_deps() {
  ...
  local pkgs_needed=()
  for pkg in usbmuxd python3 python3-pip python3-venv; do
...
lsusb 2>/dev/null | grep -q "05ac" || die "No Apple device on USB. Connect your iPhone and re-run."
...
USBMUXD_SOCKET=""
for candidate in /run/usbmuxd /var/run/usbmuxd /tmp/usbmuxd; do
  [[ -S "$candidate" ]] && { USBMUXD_SOCKET="$candidate"; break; }
done

if [[ -z "$USBMUXD_SOCKET" ]]; then
  warn "usbmuxd socket not found — starting usbmuxd..."
  sudo systemctl start usbmuxd 2>/dev/null || sudo usbmuxd -f &
  sleep 2
  for candidate in /run/usbmuxd /var/run/usbmuxd /tmp/usbmuxd; do
    [[ -S "$candidate" ]] && { USBMUXD_SOCKET="$candidate"; break; }
  done
...
if [[ "$DEVICE_LIST" == "[]" ]] || [[ -z "$DEVICE_LIST" ]]; then
  ...
  warn "  3. Has tapped 'Trust' for this Pi"
  echo ""
...
(
  ...
  while true; do
    ...
    # Also check if the runner itself is still alive; if not, stop the watchdog
    if [[ -f "$RUNNER_PID_FILE" ]]; then
...
ok "Dashboard: http://$(hostname -I | awk '{print $1}'):7070"
echo ""
